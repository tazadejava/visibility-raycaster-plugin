...
- algorithm can be run async, but modifying blocks in the server itself must be done sync
 */
public class PreciseVisibleBlocksRaycaster {
    ...
    class FOVBounds {
        ...
        public boolean isInBounds(Location loc) {
            ...
            //TODO: for more fine bound-checking, compare the horizontal and vertical angles separately; this current algorithm, however, is plenty accurate for most cases
        }
    }
    ...
    public Block[] getVisibleBlocks(Player p) {
        ...
        if(!lineOfSight.isEmpty()) {
            ...
            while (!unvisitedAirBlocks.isEmpty()) {
                timeout--;
                if(timeout % 100 == 0) {
                    if(DEBUG_MESSAGES) Bukkit.broadcastMessage("AIR BLOCKS: " + unvisitedAirBlocks.size());
                }
                if(timeout <= 0) {
